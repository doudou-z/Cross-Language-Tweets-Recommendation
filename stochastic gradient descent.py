...
# Apply converged W,Q to test data
esTweets_ranklist = {} # {en_id1: {es_id1: rank1, es_id2: rank2, ...}, ...}
Qnew_trans = np.transpose(Qnew)
# print Qnew_trans.shape

# for en_id in enTweetID_test[:100]:
en_id = enTweetID_test[0]
# print en_id
en_vec = en_corpus_sparseMat[enTweetID2rowID[en_id]]
sim_scores = [] # [(es_id1, score1),(es_id2, score2),...] 

if en_id not in esTweets_ranklist:
	esTweets_ranklist[en_id] = {}

for es_id in esTweetID2rowID:
	es_vec = es_corpus_sparseMat[esTweetID2rowID[es_id]]
	enW = np.dot(en_vec.toarray(), Wnew)
	QT_es = Qnew_trans * es_vec.transpose()
	sim_score = np.dot(enW, QT_es)[0][0]
	# print type(sim_score)
	if es_id not in sim_scores:
		sim_scores.append((es_id, sim_score))
# print len(sim_scores) 

sim_scores_sorted = sorted(sim_scores, key=itemgetter(1), reverse=True)
sim_scoresTop500 = sim_scores_sorted[:500]
# print sim_scoresTop500
for index, (es_id,score) in enumerate(sim_scoresTop500):
	# print "Arrive here"
	if es_id in tweetsPairGroup_pos[en_id]:
		print "found"
		esTweets_ranklist[en_id][es_id] = index
	if es_id not in tweetsPairGroup_pos[en_id]:
